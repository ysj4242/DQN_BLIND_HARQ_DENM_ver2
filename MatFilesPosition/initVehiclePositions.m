...
%        simValues.IVvalue=simParams.IVvalue;
        ...
        if simParams.typeOfScenario == 1 % Legacy PPP
            ...
            positionManagement.YvehicleReal = zeros(Nvehicles,1);
            for i = 1:length(right) %오른쪽 이동 차량 y좌표 지정
                lane = randi(simParams.NLanes); %1번 차선부터 3번 차선까지 랜덤 선택
                positionManagement.YvehicleReal(right(i)) = lane*simParams.roadWidth;
            end
            for i = 1:length(left) %왼쪽 이동 차량 y좌표 지정
                lane = randi([simParams.NLanes+1 2*simParams.NLanes]); %4번 차선부터 6번 차선까지 랜덤 선택
