...
[simParams,appParams,phyParams,outParams] = initiateParameters({'default','Technology','NR-V2X','TypeOfScenario','PPP', ...
    'beaconSizeBytes',B,'simulationTime',T,'rho',5,'roadLength',2000,'NLanes',3,'MCS_NR',7, ...
    ...
    'Priority_SPS',true, 'T2autonomousMode_min',5});
...
while timeManagement.timeNow < simParams.simulationTime
    ...
    if stationManagement.DENM_list(:,1)==0
        stationManagement.a = stationManagement.a + 1;
        stationManagement.prob(stationManagement.activeIDs) = rand(length(stationManagement.activeIDs),1);
        stationManagement.pckBuffer_prob(stationManagement.activeIDs) = stationManagement.prob(stationManagement.activeIDs);
        sorted_timeNextEvent = sort(timeManagement.timeNextEvent);
        for i = 1 : simValues.maxID
            if stationManagement.pckBuffer_prob(i) < simParams.DENM_prob
                stationManagement.saved(i,1) = timeManagement.timeNextEvent(i);
                stationManagement.saved(i,2) = stationManagement.resReselectionCounterCV2X(i);
                stationManagement.saved(i,3) = stationManagement.BRid(i,1);
                stationManagement.saved(i,4) = stationManagement.BRid(i,2);
                stationManagement.saved(i,5) = timeManagement.timeNextPacket(i);
                if stationManagement.b == 1 & timeManagement.timeNextPacket(i) > sorted_timeNextEvent(1)
                    timeManagement.timeNextEvent(i) = sorted_timeNextEvent(1) + 1e-15;
                    timeManagement.timeNextPacket(i) = sorted_timeNextEvent(1) + 1e-15;
                    stationManagement.DENM_list(i) = 1;
                    stationManagement.b = stationManagement.b + 1;
                elseif timeManagement.timeNextPacket(i) > sorted_timeNextEvent(1)
                    timeManagement.timeNextEvent(i) = sorted_timeNextEvent(1) + stationManagement.b * 1e-15;
                    timeManagement.timeNextPacket(i) =sorted_timeNextEvent(1) + stationManagement.b * 1e-15;
                    stationManagement.DENM_list(i) = 1;
                    stationManagement.b = stationManagement.b + 1;
                end
            end
...
%             %If this is not the case, the pckNextAttempt must be reset
        ...
        if stationManagement.DENM_list(idEvent) >= 1
            ...
            timeManagement.timeNextPacket(idEvent) = stationManagement.saved(idEvent,5);
            timeManagement.timeNextEvent(idEvent) = stationManagement.saved(idEvent,1);
        end
