...
for idLte = idLteHasPck'
    if stationManagement.DENM_pck(idLte) == 1
...
if ~isempty(stationManagement.transmittingIDsCV2X)
    for i = 1 : length(stationManagement.transmittingIDsCV2X)
 	...
 	stationManagement.pckNextAttempt(stationManagement.transmittingIDsCV2X) = stationManagement.pckNextAttempt(stationManagement.transmittingIDsCV2X) + 1; % pckNextAttempt가 4일때 그 패킷의 마지막 전송임
